...
page_bg_img = '''
            <style>
            body {
            background-image: url("https://sv1.picz.in.th/images/2022/11/01/vq0ai8.jpg");
...
#title of our app
st.title('Sentiment :hatched_chick:')
st.subheader('An interactive Web app to perform Sentiment Analysis any Hot topic, Product or Trend.')
...
if st.button("Run"):
    ...
    positive = format(t['pos'] , '.2f')
    neutral = format(t['neu'],'.2f')
    negative = format(t['neg'] , '.2f')
    
    #passing the pirchart in streamlit function as st.pyplot()
    labels = ['Positive ['+str(positive)+'%]' , 'Neutral ['+str(neutral)+'%]' , 'Negative['+str(negative)+'%]']
    sizes = [t['pos'],t['neu'],t['neg']]
...
#x = st.slider('Select the Number of Tweets',0, 1000)
#noOfsearchWord = x

#tweets = tweepy.Cursor(api.search_tweets , q = searchWord ).items(noOfsearchWord)
#print (tweets)
    ...
    #with st.spinner('Loading tweets....'):
        #for tweet in tweets:
                ...
                #print('\n')
                #tweet = text(tweet)
                #tweety = review_to_words(tweet)
                #temp = sid.polarity_scores(tweety)
